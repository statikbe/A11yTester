...
// options.
//
// https://github.com/pa11y/pa11y-ci/blob/master/package.json
"use strict";
...
commander
  .version(pkg.version)
  .usage("[options] <paths>")
  .option("-c, --config <path>", "the path to a JSON or JavaScript config file")
...
Promise.resolve()
  ...
  .then((report) => {
    ...
    if (
      report.errors >= parseInt(commander.threshold, 10) &&
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (commander.config && !config) {
        return reject(
          new Error(`The config file "${configPath}" could not be loaded`)
        );
      }
    } catch (error) {
      return reject(
        new Error(
          `There was a problem loading "${configPath}":\n${error.stack}`
        )
      );
    }
...
function loadSitemapIntoConfig(program, initialConfig) {
  const sitemapFind = program.sitemapFind
    ? new RegExp(program.sitemapFind, "gi")
    : null;
  const sitemapReplace = program.sitemapReplace || "";
  const sitemapExclude = program.sitemapExclude
